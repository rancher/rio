  ...
  CONFIG['nodes'].each_with_index do |node, i|
    config.vm.define node['name'] do |named|
      ...
      if i == 0
        ...
        named.vm.network "forwarded_port", guest: 3000, host: 3000, host_ip: "127.0.0.1"
        named.vm.network "forwarded_port", guest: 3001, host: 3001, host_ip: "127.0.0.1"
        named.vm.network "forwarded_port", guest: 3002, host: 3002, host_ip: "127.0.0.1"
        named.vm.network "forwarded_port", guest: 3003, host: 3003, host_ip: "127.0.0.1"
        named.vm.network "forwarded_port", guest: 3004, host: 3004, host_ip: "127.0.0.1"
        # detect host os
        if OS.mac?
          hostOS = "darwin"
        elsif OS.linux?
