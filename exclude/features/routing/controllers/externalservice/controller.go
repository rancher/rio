...
func Register(ctx context.Context, rContext *types.Context) error {
	...
	c.Apply.WithCacheTypes(rContext.Networking.Networking().V1alpha3().ServiceEntry(), rContext.Networking.Networking().V1alpha3().VirtualService())
...
func (p populator) populate(obj runtime.Object, stack *riov1.Stack, os *objectset.ObjectSet) error {
	...
	if obj.(*riov1.ExternalService).Spec.Service != "" {
		...
		populate.VirtualServiceForExternalService(p.systemNamespace, obj.(*riov1.ExternalService), serviceSet, svc, stack, os)
	}
	return nil
}
