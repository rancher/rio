...
func VirtualServices(systemNamespace string, stack *v1.Stack, routeSet *v1.Router, externalServiceMap map[string]*v1.ExternalService, routesetMap map[string]*v1.Router, os *objectset.ObjectSet) error {
	vs := vsFromRoutesets(systemNamespace, stack, routeSet, externalServiceMap, routesetMap, os)
	if vs != nil {
		os.Add(vs)
	}
...
func vsFromRoutesets(systemNamespace string, stack *v1.Stack, routeSet *v1.Router, externalServiceMap map[string]*v1.ExternalService, routesetMap map[string]*v1.Router, os *objectset.ObjectSet) *v1alpha3.VirtualService {
		...
		for _, dest := range routeSpec.To {
			...
			if esvc, ok := externalServiceMap[dest.Service]; ok {
				...
				routeSpec.Rewrite = &v1.Rewrite{
					Host: fmt.Sprintf("%s-%s.%s", dest.Destination.Service, namespace.StackNamespaceOnlyHash(stack.Namespace, dest.Destination.Stack), settings.ClusterDomain),
				}
...
func newVirtualServiceGeneric(stack *v1.Stack, name, namespace string) *v1alpha3.VirtualService {
