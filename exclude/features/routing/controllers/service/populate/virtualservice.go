...
func virtualServices(systemNamespace string, stack *v1.Stack, services []*v1.Service, service *v1.Service, os *objectset.ObjectSet) error {
	...
	os.Add(vsFromService(systemNamespace, stack, service.Name, serviceSet)...)

	return nil
}

func vsRoutes(systemNamespace string, publicPorts map[string]bool, service *v1.Service, dests []Dest) ([]v1alpha3.HTTPRoute, bool) {
	external := false
	...
	for _, publicDomain := range strings.Split(service.Annotations[PublicDomainAnnotation], ",") {
		...
		_, route := newRoute(domains.GetPublicGateway(systemNamespace), true, pb, ds, false, false, nil)
...
func vsFromService(systemNamespace string, stack *v1.Stack, name string, service *serviceset.ServiceSet) []runtime.Object {
	var result []runtime.Object

	serviceVS := VsFromSpec(systemNamespace, stack, name, service.Service.Namespace, service.Service, DestsForService(name, stack.Name, service)...)
	if serviceVS != nil {
		result = append(result, serviceVS)
	}

	for _, rev := range service.Revisions {
		revVs := VsFromSpec(systemNamespace, stack, rev.Name, service.Service.Namespace, rev, Dest{
			Host:   rev.Name,
			Subset: rev.Spec.Revision.Version,
			Weight: 100,
		})
...
func VsFromSpec(systemNamespace string, stack *v1.Stack, name, namespace string, service *v1.Service, dests ...Dest) *v1alpha3.VirtualService {
	publicPorts := map[string]bool{}

	routes, external := vsRoutes(systemNamespace, publicPorts, service, dests)
	if len(routes) == 0 {
		return nil
	}
...
func newVirtualService(stack *v1.Stack, service *v1.Service) *v1alpha3.VirtualService {
