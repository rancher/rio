...
func newGithubClient() (*scm.Client, error) {
...
func getHookEndpoint(receiver *webhookv1.GitWebHookReceiver) string {
	...
	return hookUrl(fmt.Sprintf("http://%s.%s", service.HashIfNeed("webhook", "webhook", "system"), settings.ClusterDomain.Get()), receiver)
}
