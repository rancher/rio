...
func destWeightForService(d v1.WeightedDestination, stack *v1.Stack) v1alpha3.DestinationWeight {
	...
	return v1alpha3.DestinationWeight{
		Destination: v1alpha3.Destination{
			Host:   fmt.Sprintf("%s.%s.svc.cluster.local", namespace.HashIfNeed(d.Service, d.Stack, stack.Namespace), settings.CloudNamespace),
			Subset: d.Revision,
...
func destWeightForExternalService(d v1.WeightedDestination, esvc *v1.ExternalService, stack *v1.Stack) v1alpha3.DestinationWeight {
	...
	if esvc.Spec.FQDN != "" {
		...
		d.Service = fmt.Sprintf("%s.%s.svc.cluster.local", namespace.HashIfNeed(serviceName, stackName, stack.Namespace), settings.CloudNamespace)
	} else if len(esvc.Spec.IPAddresses) > 0 {
		d.Service = fmt.Sprintf("%s.%s.svc.cluster.local", namespace.NameRef(esvc.Name, stack), settings.CloudNamespace)
	}
	return v1alpha3.DestinationWeight{
		Destination: v1alpha3.Destination{
			Host:   d.Service,
