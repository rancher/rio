...
func statefulSet(service *riov1.Service, cp *controllerParams, os *objectset.ObjectSet) error {
	...
	for _, volumeTemplate := range service.Spec.VolumeTemplates {
		statefulSet.Spec.VolumeClaimTemplates = append(statefulSet.Spec.VolumeClaimTemplates, volumeTemplate)
	}
