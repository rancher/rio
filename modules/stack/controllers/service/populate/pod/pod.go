...
import (
	...
	"github.com/rancher/rio/modules/stack/controllers/service/populate/container"
	"github.com/rancher/rio/modules/stack/controllers/service/populate/podvolume"
	"github.com/rancher/rio/modules/stack/controllers/service/populate/rbac"
	"github.com/rancher/rio/modules/stack/controllers/service/populate/servicelabels"
	sidekick2 "github.com/rancher/rio/modules/stack/controllers/service/populate/sidekick"
...
func rootImage(service *riov1.Service) string {
	image := service.Status.ContainerImages[service.Name]
	if image != "" {
		return image
	}
	return service.Spec.Image
}

func containers(podSpec *v1.PodSpec, service *riov1.Service) {
	rootImage := rootImage(service)
	if rootImage != "" {
		podSpec.Containers = append(podSpec.Containers, container.Container(service.Name, rootImage, service.Spec.ContainerConfig))
	}

	for _, name := range sidekick2.SortedKeys(service.Spec.Sidekicks) {
		sidekick := service.Spec.Sidekicks[name]
		c := container.Container(name, service.Status.ContainerImages[name], sidekick.ContainerConfig)
