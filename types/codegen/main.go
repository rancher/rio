...
func main() {
	if err := generator.DefaultGenerate(schema.Schemas, basePackage, false, nil); err != nil {
		logrus.Fatal(err)
	}
	if err := generator.DefaultGenerate(networkingSchema.Schemas, basePackage, false, nil); err != nil {
		logrus.Fatal(err)
	}
	if err := generator.DefaultGenerate(projectSchema.Schemas, basePackage, false, nil); err != nil {
		logrus.Fatal(err)
	}
